...
import fcampos.rawengine3D.model.Material;
import fcampos.rawengine3D.model.Obj;
import fcampos.rawengine3D.model.ObjLoader;
 ...
 * @author Fabio
 */
public class Sala3D extends GameCore {
    ...
    private Obj[] tipos = new Obj[7];
    
    private ArrayList<Obj> objetos = new ArrayList<Obj>();
        
    // Apontador para material da fonte de luz
    private Material mat_luz;
    ...
    public ObjLoader lo = new ObjLoader();
    ...
    public void init() throws IOException
    {
    	...
    	plano = lo.carregaObjeto("parede.obj",true, false);
    	...
    	mesa = lo.carregaObjeto("mesagrande.obj",true, false);
    	mesapeq = lo.carregaObjeto("mesapeq.obj",true, false);
    	cadeira = lo.carregaObjeto("cadeira2.obj",true, false);
    	quadro = lo.carregaObjeto("quadronegro.obj",true, false);
    	porta = lo.carregaObjeto("porta.obj",true, false);
    	janela = lo.carregaObjeto("janela.obj",true, false);
    	
    	lamp = lo.carregaObjeto("lampada.obj",true, false);
    	// Como o estado da lâmpada varia, não
    	// é interessante gerar display list para ela
    	lo.desabilitaDisplayList(lamp);
    	
    	vidro = lo.carregaObjeto("vidro.obj",true, false);
    	ceu  = lo.carregaObjeto("ceu2.obj",true, false);

    	// Carrega objetos que podem estar sobre as mesas
    	tipos[0] = lo.carregaObjeto("lapis.obj",true, false);
    	tipos[1] = lo.carregaObjeto("livro.obj",true, false);
    	tipos[2] = lo.carregaObjeto("papel1.obj",true, false);
    	tipos[3] = lo.carregaObjeto("papel2.obj",true, false);
    	tipos[4] = lo.carregaObjeto("papel3.obj",true, false);
    	tipos[5] = lo.carregaObjeto("cuiabomba.obj",true, false);
    	tipos[6] = lo.carregaObjeto("borracha1.obj",true, false);
    	
    	// Cria todas as display lists, exceto para
    	// as lâmpadas e para o plano
    	lo.criaDisplayList(null);
        paused = false;
        fullscreen = false;
    	...
    	mat_luz = lamp.getMaterial(lamp.findMaterial("Luz"));
    	...
    	restauraObjetos();
    	restauraCamera();
    	Vector3f max = lo.maxDist();
    	System.out.println(" X: " + max.x +" Y: " + max.y +" Z: " + max.z);
    ...
    public void update()
    {
        ...
        public void checkGameInput()
        {
            ...
            if (solid.isPressed())
            {
            	for(int i=0; i<lo.getObj().size(); i++)
            	{
            		if (lo.getObj(i).getDrawMode().equalsIgnoreCase("w")) 
            			lo.getObj(i).setDrawMode("s");
            		else if(lo.getObj(i).getDrawMode().equalsIgnoreCase("s")) 
            			lo.getObj(i).setDrawMode("t");
            		else if(lo.getObj(i).getDrawMode().equalsIgnoreCase("t")) 
            			lo.getObj(i).setDrawMode("w");
     ...
     // Desenha o "céu"
        void DesenhaCeu()
        {
        	...
        	if(ceu.getDrawMode().equalsIgnoreCase("s"))
        	{
        	...
        	else if(ceu.getDrawMode().equalsIgnoreCase("t"))
        	{
     ...
     // Desenha 4 paredes
        private void DesenhaParedes()
        {
        	...
        	**********************************/
        	...
        	if(plano.getDrawMode().equalsIgnoreCase("t"))
        	{
        		glColor3f(1,1,1);
        	}
        	else
        	...
        	plano.setTextura(parede.getTexID());
     ...
     // Desenha o chão
        private void DesenhaChao()
        {
        	...
        	plano.setTextura(chao.getTexID());
        	//lo.desenhaObjeto(plano);
        	plano.draw();
        	glPopMatrix();
     ...
     // Desenha o teto
        private void DesenhaTeto()
        {
        	...
        	plano.setTextura(teto.getTexID());
        	//lo.desenhaObjeto(plano);
        	plano.draw();
        	glPopMatrix();
